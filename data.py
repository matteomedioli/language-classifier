...
def preprocessing(df, indput_dim=16):
    # Drop Duplicates
    ...
    # Categorical Language column to ID
    print(pd.Categorical(df_clean['Language']))
    df_clean["language_int"] = pd.Categorical(df_clean['Language']).codes
    df_clean["Language"] = df_clean["language_int"]
    df_clean.drop(columns=["language_int"], inplace = True)
    # Extract senteces that exceed max_len (-2 due CLS and SEP)
    max_len = indput_dim - 2
    ...
    df_clean["Text"] = df_clean["Text"].apply(lambda sentence: sentence + [PAD for _ in range(indput_dim-len(sentence))])
